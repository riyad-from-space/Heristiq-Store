...
export default async function ReportsPage() {
  ...
  /* Same rule as the products list: alphabetical, unfiled last. */
    ...
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat
          ...
          hint={
            revenue30 > 0
              ? `${num((profit30 / revenue30) * 100, 1)}% margin`
              : "—"
          }
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title={`Needs restocking (${low.length})`}>
          {low.length === 0 ? (
            ...
            /*
             * Grouped, unlike the two tables above it.
             *
             * This is a WORKLIST — what to reorder — so gathering it by
             * category is how it gets used: one supplier, one order, one
             * aisle. Best sellers and slow movers are RANKINGS, and grouping a
             * ranking destroys the only thing it is for.
             */
            <Table head={["Product", "On hand", "Reorder at"]}>
              {lowByCategory.map(([groupName, rows]) => (
