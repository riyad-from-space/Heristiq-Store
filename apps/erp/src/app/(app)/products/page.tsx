...
/*
 * Rows grouped under their category, with a heading band between groups.
 *
 * A flat alphabetical list mixed bracelets and waist chains together, so
 * "how many bracelets are low" meant reading every row and remembering which
 * was which. Grouping answers it by looking.
 *
 * Sorted by category name, with the unfiled last — an unfiled product is a
 * tidying job rather than a department, and burying it mid-table is how it
 * stays unfiled.
 */
const UNFILED = "Not in a category";

function byCategory(products: ProductStockRow[]) {
  const groups = new Map<string, ProductStockRow[]>();
  for (const p of products) {
    const key = p.category?.trim() || UNFILED;
    ...
    <>
      ...
      >
        ...
        {products.length === 0 ? (
          ...
          >
            {byCategory(products).map(([groupName, rows]) => (
              <Fragment key={groupName}>
                {/*
                 * A heading band, inside the same <tbody> so the columns stay
                 * aligned with the rows beneath it. A separate table per
                 * category would let each one size its own columns and the
                 * numbers would stop lining up down the page.
                 */}
                <tr className="bg-neutral-50 dark:bg-neutral-900/60">
                  <td
                    colSpan={10}
                    className="px-2 py-1.5 text-xs font-semibold tracking-wide text-neutral-600 uppercase dark:text-neutral-400"
                  >
                    {groupName}{" "}
