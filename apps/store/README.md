...
Companion to **[the ERP](../erp)**, in this same repo, which stays the source of
truth for inventory, cost and profit. The storefront does not own stock.

## How the two fit together

```
                 ┌──────────────────────────────┐
  customer  ───▶ │  apps/store (this app)       │
                 ...
                 ┌───────────▼──────────────────┐
                 │  Supabase Postgres           │
                 │  products · stock ledger     │◀── apps/erp (owner)
...
| Layout | npm workspace, `apps/store` | Shares one lockfile, one migrations folder and `packages/shared` with the ERP |
...
```bash
npm install                    # from the REPO ROOT — it is one workspace
cp apps/store/.env.example apps/store/.env.local   # optional — see below
npm run dev                    # http://localhost:3000
```

`npm run dev` at the root starts this app; `npm run dev:erp` starts the ERP.
Scripts below are written from the repo root.
...
outcome this flow must not produce. `node apps/store/scripts/checkout-e2e.mjs` walks all of
it.
...
| `node apps/store/scripts/shoot.mjs /` | Screenshot a route at phone size and report horizontal overflow |
| `node apps/store/scripts/checkout-e2e.mjs` | Walk a real order through the site: add to cart → OTP → address → place → confirm |
| `node apps/store/scripts/courier-e2e.mjs` | Walk phase 4: push → webhook → replay → stale status → tracking rail |
| `node apps/store/scripts/courier-check.mjs` | Set up and verify a courier: Pathao stores, cities, sample address; Steadfast balance |
| `node apps/store/scripts/pathao-match-test.mjs` | 41 fixture cases for the address matcher, no account needed |
| `node apps/store/scripts/ship.mjs HQ-01042` | Push one order to its courier (the one-tap push, from a terminal) |
...
- [ ] **Run `node apps/store/scripts/courier-check.mjs`** once the Pathao credentials are
