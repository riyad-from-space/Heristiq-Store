...
--
-- You have already applied 0005 through 0010. This is 0011, which is the fix
-- that 0005 should have been: 0005 revoked EXECUTE from PUBLIC, but Supabase
-- grants every function an EXPLICIT privilege to `anon` as well, so the
-- privileged functions stayed callable with the public browser key.
--
-- Verified against the live project after 0005: adjust_stock entered the
-- function body and failed on a foreign key rather than on permission.
--
